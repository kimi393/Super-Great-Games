...
PITCH_SILENCE_THRESHOLD = 0.01   # RMS below this = silence
PITCH_HIGH_THRESHOLD = 350       # Hz above this = jump
...
# Level Generator
# ---------------------------------------------------------------------------
def generate_platforms(start_x, count=20):
    ...
    for _ in range(count):
        ...
        gap = random.randint(60, 160)
        dy = random.choice([-60, -40, -20, 0, 0, 20, 40])
        x += w + gap
        y = max(200, min(GROUND_Y, y + dy))
    return platforms
...
def main():
    ...
    def reset_game():
        player = Player(100, GROUND_Y - 44)
        platforms = generate_platforms(0, 40)
        particles = []
        score = 0
    ...
    while running:
        ...
        if player.alive:
            ...
            if player.x + SCREEN_WIDTH * 2 > last_plat_end:
                new_plats = generate_platforms(last_plat_end + 80, 20)
