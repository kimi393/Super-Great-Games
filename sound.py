...
def main():
    ...
    while True:  # Main menu loop
        ...
        def apply_gain(audio_chunk, gain_db=6.0):
            """Apply gain boost to quiet audio with soft clipping"""
            audio_np = np.frombuffer(audio_chunk, dtype=np.float32)
            gain_linear = 10 ** (gain_db / 20.0)
            boosted = audio_np * gain_linear
            
            # Use soft clipping (tanh) instead of hard clipping for smoother sound
            # This prevents harsh distortion when clipping occurs
            clipped = np.tanh(boosted)
            ...
            if is_playback and os.path.exists(record_filename):
                ...
                with wave.open(record_filename, 'rb') as wav_file:
                    ...
                    while is_running[0]:
                        ...
                        # Apply gain boost for better volume on low recordings
                        boosted_audio = apply_gain(frames, gain_db=1.5 + volume_gain_db[0])
                ...
                while is_running[0]:
                    try:
                        ...
                        boosted_audio = apply_gain(audio_data, gain_db=4.5 + volume_gain_db[0])
                        stream_out.write(boosted_audio)
